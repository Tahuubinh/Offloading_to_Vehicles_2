...
class DQNAgent(AbstractDQNAgent):
    ...
    """
    def __init__(self, model, file, i_name = "", policy=None, policy2=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False,
        ...
        if self.enable_double_dqn:
            self.files = open(str(file) +"/kqDDQN.csv","w")
        else:
            self.files = open(str(file) +"/kqDQN.csv","w")
...
class BDQNAgent(DQNAgent):
    def __init__(self, reward_capacity = 0, k=0.1, epsilon = 0.1, *args, **kwargs):
        ...
        self.files = open(str(self.file) +"/kqBDQN.csv","w")
